...
///
/// A writer thread starts by first claiming one or more slots in the ring
/// buffer by calling one of the 'claim' methods. The writer then writes to
/// the slots and indicates to readers it has finished writing to the slots by
/// calling publish().
///
...
///
/// \tparam WaitStrategy
/// The strategy to use to block threads waiting for sequence numbers to be
/// published.
template <typename WaitStrategy> class single_threaded_claim_strategy {
public:
  /// \brief
  ...
  single_threaded_claim_strategy(size_t bufferSize, WaitStrategy &waitStrategy)
      : m_bufferSize(bufferSize), m_nextSequenceToClaim(0),
        m_claimBarrier(waitStrategy), m_readBarrier(waitStrategy) {
  ...
  /// were available but will contain at least one slot if \p count is
  /// non-zero.
  ...
  void publish(sequence_t sequence) { m_readBarrier.publish(sequence); }
  ...
  ///
  /// This operation has 'acquire' memory semantics.
  sequence_t last_published() const { return m_readBarrier.last_published(); }

...
template <typename WaitStrategy> class sequence_barrier {
public:
  /// \brief
  ...
  /// \brief
  /// Block the calling thread until the specified sequence number is
  /// published.
  ///
  /// This operation synchronises with the corresponding call to publish()
  ...
  ///
  /// \throws std::exception
  /// May throw any exception thrown by the
  /// WaitStrategy::wait_until_published() function.
  ...
  ///
  /// This operation synchronises with calls to any of the
  /// wait_until_published() overloads that wait on the specified \p sequence.
  ...
  /// \throws std::exception
  /// May throw any exception thrown by the
  /// WaitStrategy::signal_all_when_blocking() method.

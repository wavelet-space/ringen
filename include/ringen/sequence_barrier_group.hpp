...
template <typename WaitStrategy> class sequence_barrier_group {
public:
  /// \brief
  /// Initialise the sequence barrier group to the empty set of sequence
  /// barriers.
  ///
  /// \note
  ...
  /// group. The constructed sequence barrier will hold a reference to this
  /// object so callers must ensure the lifetime of the wait strategy exceeds
  /// that of the sequence barrier group.
  ...
  void add(const sequence_barrier_group<WaitStrategy> &barrierGroup) {
    m_sequences.insert(m_sequences.end(), barrierGroup.m_sequences.begin(),
  ...
  /// This is guaranteed to satisfy <tt>difference(result, sequence) >=
  /// 0</tt>.
  ///
  /// \throw std::exception
  ...
  sequence_t wait_until_published(sequence_t sequence) const {
    ...
    sequence_t current =
        minimum_sequence_after(sequence, count, m_sequences.data());
    if (difference(current, sequence) >= 0) {
      return current;
    }

    return m_waitStrategy.wait_until_published(sequence, count,
                                               m_sequences.data());
  }

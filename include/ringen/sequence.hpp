...
namespace ringen {
  ...
  /// <tt>(1 << (sizeof(sequence_t) * 8 - 2)) - 1</tt> different from each
  /// other.
  ///
  ...
  /// ie. the sequence number, \c s, such that <tt>difference(s, sequences[i])
  /// >= 0</tt> for all \c i in \[0, count)\[.
  inline sequence_t minimum_sequence(
      size_t count, const std::atomic<sequence_t>* const sequences[]) {
    assert(count > 0);
  ...
  /// \param sequences An array of \p count pointers to sequence_t values to
  /// read. \return If no sequence numbers in \p sequences precede the \p
  /// minimum sequence number then the return value is the minimum of the
  /// sequence numbers. If any sequence numbers precede \p minimum then the
  /// return value is one of the sequence numbers that precedes \p minimum.
  inline sequence_t minimum_sequence_after(
      sequence_t minimum, size_t count,
      const std::atomic<sequence_t>* const sequences[]) {
    assert(count > 0);
    sequence_diff_t minDelta =
        difference(sequences[0]->load(std::memory_order_acquire), minimum);

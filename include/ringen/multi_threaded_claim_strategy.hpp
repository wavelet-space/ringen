...
template <typename WaitStrategy> class multi_threaded_claim_strategy {
public:
  /// \brief
  ...
  /// be published.
  multi_threaded_claim_strategy(size_t bufferSize, WaitStrategy &waitStrategy)
      : m_indexMask(bufferSize - 1), m_bufferSize(bufferSize),
        m_waitStrategy(waitStrategy), m_claimBarrier(waitStrategy),
    ...
    for (sequence_t i = 0; i < bufferSize; ++i) {
      m_published[i].store(static_cast<sequence_t>(i - bufferSize),
  ...
  /// object as the claim strategy was constructed with. \note This operation
  /// is not thread-safe and the caller must ensure that no other threads are
  /// accessing the claim strategy concurrently with this call.
  ...
  /// strategy, so the caller must ensure the lifetime of the sequence
  /// barriers exceed that of the cliam strategy. This barrier must have been
  /// constructed with the same wait strategy object as the claim strategy was
  /// constructed with.
  ///
  /// \note
  /// This operation is not thread-safe and the caller must ensure that no
  /// other threads are accessing the claim strategy concurrently with this
  /// call.
  ...
  sequence_t claim_one() {
    sequence_t sequence =
        m_nextClaimable.fetch_add(1, std::memory_order_relaxed);
  ...
  sequence_range claim(size_t count) {
    ...
    sequence_t sequence =
        m_nextClaimable.fetch_add(count, std::memory_order_relaxed);
  ...
  /// \brief
  /// Attempt to claim up to \p count slots in the ring buffer without
  /// blocking.
  ///
  /// This operation has 'acquire' memory semantics.
  ///
  ...
  /// Returns \c false if no elements were claimed. The \p range out-parameter
  ...
  bool try_claim(size_t count, sequence_range &range) {
    ...
    do {
      ...
      count = std::min(count, static_cast<size_t>(diff + 1));
        ...
        sequence, static_cast<sequence_t>(sequence + count),
  ...
  ///
  /// \param range [out]
  /// Receives the range of sequence numbers claimed if the operation
  /// succeeds. Value is left unchanged if no slots were claimed.
  ///
  /// \param timeout
  ...
  bool try_claim_for(size_t count, sequence_range &range,
                     const std::chrono::duration<Rep, Period> &timeout) {
    return try_claim_until(count, range,
  ...
  bool
  try_claim_until(size_t count, sequence_range &range,
    ...
    do {
      ...
      if (diff < 0) {
        published = static_cast<sequence_t>(
            m_claimBarrier.wait_until_published(
                static_cast<sequence_t>(sequence - m_bufferSize), timeoutTime) +
            m_bufferSize);
      ...
      reducedCount = std::min(count, static_cast<sequence_t>(diff + 1));
        ...
        sequence, static_cast<sequence_t>(sequence + reducedCount),
        std::memory_order_relaxed, std::memory_order_relaxed));
  ...
  sequence_t wait_until_published(sequence_t sequence,
  ...
  sequence_t wait_until_published(
      sequence_t sequence, sequence_t lastKnownPublished,
      const std::chrono::duration<Rep, Period> &timeout) const {
    return wait_until_published(sequence, lastKnownPublished,
...
private:
  bool is_published(sequence_t sequence) const {
    return m_published[sequence & m_indexMask].load(
